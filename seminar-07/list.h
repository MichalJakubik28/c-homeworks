 ...
 * B) if ⟨list->head⟩ is not ⟨NULL⟩, then ⟨list->head->prev⟩ is ⟨NULL⟩,
 * C) if ⟨list->tail⟩ is not ⟨NULL⟩, then ⟨list->tail->next⟩ is ⟨NULL⟩,
 *
...
/**
 * \brief Predicate that tells whether the \a list is empty or not.
 *
...
/**
 * \brief Removes the first element stored in the list.
 *
 * If \p data is not \c NULL, contents of the node data will be copied
 * to the memory pointed to by the \p data pointer.
 *
...
/**
 * \brief Removes the last element stored in the list.
 *
 * If \p data is not \c NULL, contents of the node data will be copied
 * to the memory pointed to by the \p data pointer.
 *
