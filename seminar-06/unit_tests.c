...
TEST(dyn_strjoin)
{
    const char *hello = "Ahoj ";
    const char *world = "svet!";

    SUBTEST(two_words)
    {
        char *result = dyn_strjoin(hello, world);
        CHECK(result != NULL);
    ...
    SUBTEST(first_null)
    {
        FAILS_PRECONDITION(dyn_strjoin(NULL, hello));
    }

    SUBTEST(second_null)
    {
        FAILS_PRECONDITION(dyn_strjoin(hello, NULL));
    }
}
