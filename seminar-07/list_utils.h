...
/**
 * \brief Inserts a new element at position specified by the \p index from the
 * head.
 *
 * The original element at the specified index will be moved after the new one.
 *
 * \param index     Distance from the list's head.
 * \param data      Pointer to the memory containing the element.
 *
 * \note If \p index is equal to \c list_size(&list), then the new element
 ...
 *
 * \note The function compares contents of the memory the \p data pointer is
 * pointing to, not the pointers themselves.
 *
...
/**
 * \brief Returns the first element that satisfies the predicate \p p
...
/**
 * \brief Returns the last element that satisfies the predicate \p p
 * or \c NULL if no such element exists.
 *
 * \param p         The predicate.
 */
...
/**
 * \brief Calls the function \p f on each element from head to tail.
 *
...
/**
 * \brief Calls the function \p f on each element from tail to head.
 *
 * \note    The loop stops when all nodes are processed or when the action
 *          returns a value other than ⟨ACTION_CONTINUE⟩.
 *
...
/**
 * \brief Sorts all elements in the list using the \p cmp comparator.
 *
